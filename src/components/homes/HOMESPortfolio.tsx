              ...
              >
                ...
                <CardHeader className="relative pb-4">
                  <div className="flex items-center justify-between mb-6">
                    <div className={`relative p-4 rounded-2xl border transition-all duration-300 group-hover:scale-105 ${
                      isPrimary 
                      ...
                      <div className={`absolute -top-1 -right-1 w-4 h-4 rounded-full flex items-center justify-center ${
                        ...
                        <card.accentIcon className={`w-2.5 h-2.5 ${
                    ...
                    <div className="flex flex-col items-end space-y-1">
                      <div className={`h-1 w-8 rounded-full ${
                        isPrimary 
                          ? 'bg-gradient-to-r from-transparent to-header-glow/30' 
                          : 'bg-gradient-to-r from-transparent to-muted-foreground/20'
                      }`}></div>
                      <div className={`h-1 w-6 rounded-full ${
                        isPrimary 
                          ? 'bg-gradient-to-r from-transparent to-header-glow/20' 
                          : 'bg-gradient-to-r from-transparent to-muted-foreground/15'
                      }`}></div>
                      <div className={`h-1 w-4 rounded-full ${
                        isPrimary 
                          ? 'bg-gradient-to-r from-transparent to-header-glow/10' 
                          : 'bg-gradient-to-r from-transparent to-muted-foreground/10'
                      }`}></div>
                    </div>
                  </div>
                  <CardTitle className="text-2xl font-bold mb-2 tracking-tight">{card.title}</CardTitle>
                  <p className="text-muted-foreground text-sm font-medium">{card.subtitle}</p>
                </CardHeader>
                
                <CardContent className="relative space-y-6">
                  {/* Primary Stat with unified styling */}
                  <div className="text-center py-6 px-4 rounded-2xl bg-background/60 backdrop-blur-sm border border-border/50">
                  ...
                  {card.progressValue && isPrimary && (
                    <div className="space-y-3">
                      <div className="relative w-full bg-muted/30 rounded-full h-4 overflow-hidden border border-border/50">
                        <div 
                          className="bg-gradient-to-r from-header-glow via-header-glow/80 to-header-glow h-4 rounded-full transition-all duration-1000 ease-out shadow-lg relative"
                      ...
                      <div className="flex justify-between text-xs text-muted-foreground font-medium">
                        <span>0%</span>
                        <span className="text-header-glow">{card.progressValue}% Complete</span>
                        <span>100%</span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {card.tertiaryStats.map((stat, statIndex) => (
                      <div key={statIndex} className="p-3 rounded-xl bg-background/40 backdrop-blur-sm border border-border/30">
                  ...
                  {/* Secondary stat */}
                  <div className="flex justify-between items-center py-4 px-5 rounded-xl bg-background/50 backdrop-blur-sm border border-border/40">
                    <span className="text-sm text-muted-foreground font-medium">{card.secondaryStat.label}</span>
                    <span className="font-bold text-lg">{card.secondaryStat.value}</span>
                  </div>
                  
                  {/* CTA Button with unified styling */}
                  <Button 
                    className={`w-full py-6 text-lg font-bold rounded-2xl transition-all duration-300 group/btn ${
                      isPrimary
