...
const BuyCard = ({ canBuy }: { canBuy: boolean }) => {
  ...
  const handleSwap = () => {
    ...
    if (
      ...
      receiveAmount.tokens > balance(sellAsset)
    ) {
      toast({
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-4 mb-8">
        <ArrowUpDown className="size-6 md:size-5 text-header-glow" />
          ...
          .with(true, () => (
            <div className="bg-background/30 rounded-2xl border border-border/20 p-4 md:p-6 space-y-6 max-w-3xl w-full">
              ...
              <div className="space-y-4">
                ...
                <Select
                  value={buyAsset}
                  onValueChange={(value) => setBuyAsset(value)}
                >
                  <SelectTrigger className="bg-muted/50 h-12 md:h-auto font-sans">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <div className="flex items-center bg-muted/30 rounded-md p-1 gap-1 min-w-0">
                      <Button
                        ...
                        className={`h-7 text-xs px-2 min-w-[50px] ${
                          denomination === "usd"
                            ? "btn-hastra"
              ...
              {amount && receiveAmount.tokens > 0 && (
                <div className="bg-background/30 border border-orange-800/30 rounded-xl p-4 md:p-6">
                  <div className="text-sm md:text-sm text-muted-foreground mb-2">
                    You'll receive
                  </div>
                  <div className="font-semibold text-lg md:text-base text-[hsl(34_100%_84%)]">
                    {receiveAmount.tokens.toFixed(2)} {symbol(buyAsset)}
                  </div>
                  <div className="text-xs md:text-xs text-muted-foreground">
                    ${receiveAmount.usd.toFixed(2)} USD
                  </div>
                  {txId && (
