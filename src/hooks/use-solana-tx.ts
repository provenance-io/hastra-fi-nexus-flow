...
const confirmTransaction = async (connection: Connection, fn: () => Promise<TransactionSignature>):
...
export const useDepositAndMint = () => {
    ...
    const depositAndMint = useCallback(async (amount: number) => {
        if (!wallet) {
        ...
        const configPda = new PublicKey(import.meta.env.VITE_SOLANA_USDC_YIELD_CONFIG_PDA);
        const mintAuthorityPda = new PublicKey(import.meta.env.VITE_SOLANA_USDC_YIELD_MINT_AUTHORITY_PDA);

        return await confirmTransaction(connection, () => program?.methods.depositAndMint(new BN(amount * 1_000_000))
            .accounts(
                {
                    signer: signer,
...
export const useAirDrop = () => {
    const fundWithAirdrop = useCallback(async (publicKey: PublicKey | null) => {
        ...
        return confirmTransaction(connection, () => connection.requestAirdrop(publicKey, 0.1 * LAMPORTS_PER_SOL));
    }, []);
...
export const useCreateTokenAccounts = () => {
    ...
    const createTokenAccount = useCallback(
        async (mintAddresses: string[]) => {
            ...
            const neededAtas = await Promise.all(mintPublicKeys.map(async (mint, index) => {
                const tokenAddress = await getAssociatedTokenAddress(mintPublicKeys[index], signer);
            ...
            return confirmTransaction(connection, () => connection.sendRawTransaction(signed.serialize(), {skipPreflight: false}));
        },
