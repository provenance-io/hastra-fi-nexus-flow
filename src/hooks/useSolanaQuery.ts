...
export const useAtaQuery = (publicKey: PublicKey | null, tokenAddress?: string) => {
    return useQuery<string, Error>({
        queryKey: ['ata', publicKey, tokenAddress],
        enabled: !!publicKey && !!tokenAddress,
        queryFn: async () => {
            if (!publicKey || !tokenAddress) throw new Error('No wallet or token address provided');
            const ta = await getAssociatedTokenAddress(new PublicKey(tokenAddress), publicKey);
            return ta.toBase58();
        }
    });
};
...
export const useAtaExistsQuery = (publicKey: PublicKey | null, tokenAddress: string) => {
    return useQuery<boolean, Error>({
...
export const useAtaBalanceQuery = (publicKey: PublicKey | null, tokenAddress: string) => {
    return useQuery<number, Error>({
